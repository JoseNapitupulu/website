...
async function generate() {
  ...
  // read generated PNGs as buffers and pass to png-to-ico
  const png16 = fs.readFileSync(path.join(outDir, 'favicon-16.png'));
  const png32 = fs.readFileSync(path.join(outDir, 'favicon-32.png'));
  const png180 = fs.readFileSync(path.join(outDir, 'favicon-180.png'));
  const icoBuffer = await pngToIco([png16, png32, png180]);
